...
function ExcelReader() {
  const { excelData, baseUrl, setSelected, selected } =
  ...
  const sendDataToAPI = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${baseUrl}/api/Admin/CreateMultiTrip`,
        formattedData,
        {
          headers: {
  ...
  return (
    <div>
          ...
          {formattedData
            ...
            .map((item, index) => (
              ...
              >
                ...
                <div className="col-md-1">
                  {googleLoading && item.destination ? (
                    "Loading"
                  ) : (
                    <div>{item.destination?.address}</div>
                  )}
                </div>
                <div className="col-md-1">
                  {item.specialNeeds ? (
                ...
                <div className="col-md-1">
                  ...
                  ></i>
                  <i
                    className="fa-solid fa-pencil icon-color"
                    onClick={() => {
      ...
      >
        <div>
          ...
          <button
            ...
            className="btn-bg btn ms-auto d-block w-25 my-3 mx-3 fw-bold"
          >
            Update
          </button>
        </div>
        <div className="text-center my-3">
          {" "}
          <button className="btn modal-close-btn px-3 " onClick={closeModal}>
            Okay
          </button>
        </div>
      </Modal>
      ...
      <div className="text-end  me-5">
        {" "}
        <button className="btn btn-bg" onClick={sendDataToAPI}>
          {loading ? <Loading /> : "SendDataToApi"}
        </button>
      </div>
    </div>
  );
