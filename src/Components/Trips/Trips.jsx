...
export default function Trips() {
  ...
  let { baseUrl, setSelected, excelData, setExcelData } =
  ...
  async function getPage(currentPage) {
    ...
    try {
      const data = await axios.get(
        `${baseUrl}/api/Admin/Trips/${currentPage}?date=${selectedDate}`,
        {
          headers: {
  ...
  async function getPageCount() {
    try {
      const data = await axios.get(
        `${baseUrl}/api/Admin/Trips/PagesCount?date=${selectedDate}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      <div className="container-fluid bg-light">
        <div className="d-flex input-child justify-content-between pt-3 widdth m-auto">
          ...
          <div>
            ...
            />
            <Link className="btn btn-bg margin-btn" onClick={handleUploadClick}>
              Upload Excel File
            </Link>
          </div>
          ...
          {loading ? (
            ...
            trips.map((item) => (
              ...
              >
                ...
                <div className="col-md-1">
                  {item.pickup.address.split(" ").slice(0, 3).join(" ")}
                </div>
                <div className="col-md-1">
                  {item.destination.address.split(" ").slice(0, 3).join(" ")}
                </div>
                <div className="col-md-1">
                  {item.driver == null ? (
                    <div>Not assign</div>
                ...
                <div className="col-md-1">
                  ...
                  <Link
                    ...
                    to={{ pathname: `/update/updatetrip/${item._id}` }}
